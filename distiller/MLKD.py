"""
@author supermantx
@date 2024/5/28 13:43
"""
import torch
import torch.nn.functional as F
import numpy as np
from ._base import Distiller_Origin
...
class MLKD(Distiller_Origin):
    ...
    def forward_train(self, image_weak, image_strong, **kwargs):
        logits_student_weak = self.student(image_weak)
        logits_student_strong = self.student(image_strong)
        with torch.no_grad():
            logits_teacher_weak = self.teacher(image_weak)
            logits_teacher_strong = self.teacher(image_strong)
        ...
        return logits_teacher_weak, logits_student_weak, losses_dict
